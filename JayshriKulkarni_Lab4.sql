...
select * from  order1 where order1.ORD_AMOUNT>=3000;

#3 Display the total number of cutomers based on the geneder who have placed orders of worth at least Rs. 3000
...
#Q.5 Display the Supplier details of who is supplying more than one product.
...
select product.pro_id,product.pro_name from order1 inner join supplier_pricing on supplier_pricing.pricing_id=order1.
pricing_id inner join product on product.pro_id=supplier_pricing.pro_id where order1.ord_date>"2021-10-05";
...
#For Type_Of_Serive,if rating =5, print "Excellent Service”, if rating >4 print “Good Service”, 
